...
Usage: python adw_plan_build.py <github-issue-number> [adw-id]
...
def parse_args(logger: Optional[logging.Logger] = None) -> Tuple[str, Optional[str]]:
    ...
    if len(sys.argv) < 2:
        usage_msg = [
            "Usage: python adw_plan_build.py <issue-number> [adw-id]",
            "Example: python adw_plan_build.py 123",
            "Example: python adw_plan_build.py 123 abc12345",
        ]
        if logger:
...
def git_branch(
    ...
    request = AgentTemplateRequest(
        ...
        args=[issue_type, issue.model_dump_json(by_alias=True)],
        adw_id=adw_id,
        model="sonnet",
    )
...
def pull_request(
    ...
    request = AgentTemplateRequest(
        ...
        args=[branch_name, issue.model_dump_json(by_alias=True), plan_file],
        adw_id=adw_id,
        model="sonnet",
    )
...
def check_error(
    ...
    if error:
        ...
        make_issue_comment(
            issue_number,
            format_issue_message(adw_id, agent_name, f"Error: {error_prefix}: {error}"),
        )
        sys.exit(1)
