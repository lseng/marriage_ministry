...
Usage:
cd adws && source venv/bin/activate && python health_check.py
...
4. Checks Supabase local instance
...
def check_env_vars() -> CheckResult:
    ...
    optional_vars = {
        ...
        "VITE_SUPABASE_URL": "(Optional) Supabase URL for local development",
...
def check_git_repo() -> CheckResult:
    ...
    try:
        ...
        return CheckResult(
            success=False,
            error=str(e),
            warning="Git repository not configured. Run 'git init' and add a remote."
        )
...
def check_supabase() -> CheckResult:
    ...
    try:
        ...
        is_running = result.returncode == 0 and "API URL" in result.stdout

        return CheckResult(
            success=True,  # CLI installed is enough for success
            warning="Local Supabase not running. Start with: supabase start" if not is_running else None,
            details={
                "cli_version": version,
                "local_running": is_running,
            },
        )
...
def check_node_deps() -> CheckResult:
    ...
    key_deps = ["@playwright/test", "vitest", "@testing-library/react"]
    missing = []
    ...
    if missing:
        return CheckResult(
            success=False,
            error=f"Missing dependencies: {', '.join(missing)}. Run: npm install",
        )

    return CheckResult(
        success=True,
        details={"node_modules": True, "key_deps_installed": True},
    )
...
def run_health_check() -> HealthCheckResult:
    ...
    node_check = check_node_deps()
    result.checks["node_dependencies"] = node_check
    ...
    result.checks["supabase"] = supabase_check
    if not supabase_check.success:
        result.success = False
        if supabase_check.error:
            result.errors.append(supabase_check.error)
    elif supabase_check.warning:
        result.warnings.append(supabase_check.warning)
...
def main():
    ...
    if not result.success:
        ...
        if any("ANTHROPIC_API_KEY" in e for e in result.errors):
            print("   1. Set ANTHROPIC_API_KEY in your .env.local file")
        ...
        if any("Node" in e for e in result.errors):
            print("   4. Install dependencies: npm install")
        if any("Supabase CLI" in e for e in result.errors):
            print("   5. Install Supabase: brew install supabase/tap/supabase")
